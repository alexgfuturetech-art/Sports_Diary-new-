#!/usr/bin/env python3
"""
Seed script to add 10+ Sports Academies to MongoDB Atlas
Usage: python seed_academies.py
"""
...
try:
    result = db.academies.insert_many(academy_docs)
...
# Verify
try:
    count = db.academies.count_documents({"category": "Academy", "is_active": True})
    print(f"\n[INFO] Total active academies in database: {count}")
    
    if count >= 10:
        print("[SUCCESS] All 10 academies are now in the database!")
        print("\n[NEXT] Refresh your browser to see all academies!")
    else:
